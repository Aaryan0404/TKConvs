...
output_gemm = output_gemm.view(N, P, Q, K)

# print shapes
print("A matrix shape (TXT INPUT): ", input_gemm.shape)
...
# print avg mag of output_data
print("1/100 PyTorch Conv avg mag: ", torch.mean(torch.abs(output_data)).item() / 100)
...
with open(fn, 'w') as f:
    ...
    for i in trange(weights_gemm.shape[0] * weights_gemm.shape[1]):
        f.write(repr(bf[i]))
        f.write(' ')
    for i in trange(output_gemm.shape[0] * output_gemm.shape[1] * output_gemm.shape[2] * output_gemm.shape[3]):
        f.write(repr(cf[i]))
        f.write(' ')
